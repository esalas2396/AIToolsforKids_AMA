...
TOPIC_IMAGES = {
    "Astronomy": os.path.join(current_dir, "AMA_UI", "astronomy.png"),
    "Sneakers and Shoes": os.path.join(current_dir, "AMA_UI", "sneakers.png"),
    "Dinosaurs": os.path.join(current_dir, "AMA_UI", "dinosaurs.png"),
    "Robots": os.path.join(current_dir,"AMA_UI", "robo.png"),
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        def update_image(event):
            ...
            # topic_var.set(f"Ask me anything about {selected_topic}:") ## didn't work for dynamic lable
        ...
        # define some initial conversation history
        conversation_history = []
        ...
        def generate_response(user_input, age, topic):
            ...
            # append user input and the corresponding AI response to conversation history file
        ...
        def button_click():
            ...
            # update the image based on the selected topic
            ...
            # append user input and modified AI  response to conversation history
        ...
        def clear():
            ...
            # self.subject_image = customtkinter.CTkImage(Image.open(os.path.join( "AMA_UI","blue.jpg")), size=(100, 100)) # <------ this changes the image back to transparent 
            # self.subject_image_label.configure(image=self.subject_image) 

        def reset():
            # Clear age and topic selection
            self.name_entry.delete(0, tk.END)
            self.age_combobox.set(" ")
            self.subject_combobox.set(" ")
            self.grade_combobox.set(" ")

            self.subject_image = customtkinter.CTkImage(Image.open(os.path.join( "AMA_UI","blue.jpg")), size=(240, 200)) # <------ this changes the image back to transparent 
            self.subject_image_label.configure(image=self.subject_image) 

            # Clear global conversation history variable for next user
        ...
        self.subject_image = customtkinter.CTkImage(Image.open(os.path.join("AMA_UI","blue.jpg")), size=(240,220) )
        ...
        self.robo_image_label = customtkinter.CTkLabel(master=self.logo_frame, image=self.logo_image, compound="left", text="")
        ...
        self.subject_combobox = customtkinter.CTkComboBox(master=self.input_frame,values=["Astronomy", "Sneakers and Shoes","Dinosaurs", "Robots"])
        self.subject_combobox.grid(row=6, column=1, padx=20, pady=(10, 10))
        self.subject_combobox.bind("<<ComboboxSelected>>", update_image)

        self.subject_image_label = customtkinter.CTkLabel(master=self.input_frame, image=self.subject_image, compound="left", text = "")  ## <-------- styling for subject/topic image ; seperate from robot logo
